...
- `include/sys/qat.h`
  ...
  - Defines the compression offload minimum as `QAT_DC_MIN_BUF_SIZE = 8 KiB`, the default maximum as `QAT_DC_DEFAULT_MAX_BUF_SIZE = 128 KiB`, and the experimental absolute maximum as `QAT_DC_ABS_MAX_BUF_SIZE = 1 MiB`.
...
`zfs_qat_dc_max_buf_size` defaults to `128 KiB` and accepts only `128 KiB`, `256 KiB`, `512 KiB`, or `1 MiB`; records above the configured value use software gzip fallback.
...
- Compression offload is only considered within the `8 KiB` to `zfs_qat_dc_max_buf_size` window, defaulting to `128 KiB`.
