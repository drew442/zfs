...
| Driver timing matrix | Ran 128K/256K/512K/1M with jobs 1/4/8 using a temporary 1M profile recordsize so all records offloaded to QAT. | QAT response wait closely tracks ZFS wait and is the dominant cost. QAT saves CPU but remains slower than software in concurrent rows; see `qat-driver-timing-matrix-20260519.md`. |
...
| Request-overhead counters | Added sync path, page-array path, buffer-list allocation, request allocation, and per-request derived benchmark fields. | Narrow `JOBS=4` baseline showed allocation overhead is small relative to QAT wait time. Next target should be QAT service-time/request-shape behavior, especially source scatter/gather shape. See `request-overhead-baseline-20260522.md`. |
