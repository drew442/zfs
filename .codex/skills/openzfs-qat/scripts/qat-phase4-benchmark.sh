...
derived_metrics_csv() {
	...
	local accel_checks_delta="${13}"
	local accel_eligible_delta="${14}"
	local accel_skip_disabled_delta="${15}"
	local accel_skip_runtime_delta="${16}"
	local accel_skip_uninit_delta="${17}"
	local accel_skip_min_delta="${18}"
	local accel_skip_max_delta="${19}"
	local async_submit_fails_delta="${20}"
	local async_fail_retry_delta="${21}"
	local async_fail_resource_delta="${22}"
	local async_fail_other_delta="${23}"
	...
	awk -v total_bytes="$total_bytes" \
	    ...
	    -v accel_checks_delta="$accel_checks_delta" \
	    -v accel_eligible_delta="$accel_eligible_delta" \
	    -v accel_skip_disabled_delta="$accel_skip_disabled_delta" \
	    -v accel_skip_runtime_delta="$accel_skip_runtime_delta" \
	    -v accel_skip_uninit_delta="$accel_skip_uninit_delta" \
	    -v accel_skip_min_delta="$accel_skip_min_delta" \
	    -v accel_skip_max_delta="$accel_skip_max_delta" \
	    -v async_submit_fails_delta="$async_submit_fails_delta" \
	    -v async_fail_retry_delta="$async_fail_retry_delta" \
	    -v async_fail_resource_delta="$async_fail_resource_delta" \
	    -v async_fail_other_delta="$async_fail_other_delta" '
	function pct(n, d) {
		return d > 0 ? sprintf("%.2f", 100 * n / d) : "na"
	}
	function capped_pct(n, d) {
		if (d <= 0)
			return "na"
		p = 100 * n / d
		if (p > 100)
	...
	BEGIN {
		...
		profile_skip = accel_skip_min_delta + accel_skip_max_delta
		runtime_skip = accel_skip_disabled_delta + accel_skip_runtime_delta + accel_skip_uninit_delta
		async_failure = async_submit_fails_delta + async_fail_retry_delta + async_fail_resource_delta + async_fail_other_delta

		printf "%s,%.2f,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s",
		    cpu_count,
		    cpu_active_pct,
		    seconds_per_gib(cpu_active_s, total_bytes),
		    seconds_per_gib(cpu_system_s, total_bytes),
		    capped_pct(comp_in_delta, total_bytes),
		    ...
		    pct(accel_eligible_delta, accel_checks_delta),
		    pct(profile_skip, accel_checks_delta),
		    pct(runtime_skip, accel_checks_delta),
		    pct(async_failure, async_submits_delta),
...
run_one() {
	...
	local page_array_slot_src_before
	local page_array_slot_src_after
	...
	local page_array_slot_dst_before
	local page_array_slot_dst_after
	...
	local page_array_slot_scratch_before
	local page_array_slot_scratch_after
	...
	local req_slot_before
	local req_slot_after
	...
	accel_checks_before="$(statv dc_compress_accel_checks)"
	accel_eligible_before="$(statv dc_compress_accel_eligible)"
	accel_skip_disabled_before="$(statv dc_compress_accel_skip_disabled)"
	accel_skip_runtime_before="$(statv dc_compress_accel_skip_runtime)"
	accel_skip_uninit_before="$(statv dc_compress_accel_skip_uninit)"
	accel_skip_min_before="$(statv dc_compress_accel_skip_min)"
	accel_skip_max_before="$(statv dc_compress_accel_skip_max)"
	...
	page_array_slot_src_before="$(statv dc_compress_page_array_slot_src)"
	...
	page_array_slot_dst_before="$(statv dc_compress_page_array_slot_dst)"
	...
	page_array_slot_scratch_before="$(statv dc_compress_page_array_slot_scratch)"
	...
	req_slot_before="$(statv dc_compress_req_slot)"
	...
	accel_checks_after="$(statv dc_compress_accel_checks)"
	accel_eligible_after="$(statv dc_compress_accel_eligible)"
	accel_skip_disabled_after="$(statv dc_compress_accel_skip_disabled)"
	accel_skip_runtime_after="$(statv dc_compress_accel_skip_runtime)"
	accel_skip_uninit_after="$(statv dc_compress_accel_skip_uninit)"
	accel_skip_min_after="$(statv dc_compress_accel_skip_min)"
	accel_skip_max_after="$(statv dc_compress_accel_skip_max)"
	...
	page_array_slot_src_after="$(statv dc_compress_page_array_slot_src)"
	...
	page_array_slot_dst_after="$(statv dc_compress_page_array_slot_dst)"
	...
	page_array_slot_scratch_after="$(statv dc_compress_page_array_slot_scratch)"
	...
	req_slot_after="$(statv dc_compress_req_slot)"
	...
	derived_csv="$(derived_metrics_csv "$total_bytes" "$elapsed_ms" \
	    ...
	    "$((accel_checks_after - accel_checks_before))" \
	    "$((accel_eligible_after - accel_eligible_before))" \
	    "$((accel_skip_disabled_after - accel_skip_disabled_before))" \
	    "$((accel_skip_runtime_after - accel_skip_runtime_before))" \
	    "$((accel_skip_uninit_after - accel_skip_uninit_before))" \
	    "$((accel_skip_min_after - accel_skip_min_before))" \
	    "$((accel_skip_max_after - accel_skip_max_before))" \
	...
	raw_row=(raw "$mode" "$verify_mode" "$record" "$iter" "$JOBS"
	    ...
	    "$((page_array_slot_scratch_after - page_array_slot_scratch_before))"
	    ...
	    "$req_alloc_delta"
	    "$req_free_delta"
	    "$((req_slot_after - req_slot_before))"
	    ...
	    "$QAT_DC_SHAPE_STATS" "$QAT_DC_EFFECTIVE_SHAPE_STATS"
	    "$QAT_DC_TIMING_STATS" "$QAT_DC_EFFECTIVE_TIMING_STATS"
...
QAT_DC_SHAPE_STATS="$(read_param zfs_qat_dc_shape_stats)"
QAT_DC_TIMING_STATS="$(read_param zfs_qat_dc_timing_stats)"
...
if [[ "$QAT_DC_SHAPE_STATS" == "profile" ]]; then
	QAT_DC_EFFECTIVE_SHAPE_STATS="0"
else
	QAT_DC_EFFECTIVE_SHAPE_STATS="$QAT_DC_SHAPE_STATS"
fi
if [[ "$QAT_DC_TIMING_STATS" == "profile" ]]; then
	QAT_DC_EFFECTIVE_TIMING_STATS="0"
else
	QAT_DC_EFFECTIVE_TIMING_STATS="$QAT_DC_TIMING_STATS"
fi
...
run_mode_record() {
	...
	declare -A summary_row=(
	    ...
	    [zfs_qat_dc_shape_stats]="$QAT_DC_SHAPE_STATS"
	    [zfs_qat_dc_effective_shape_stats]="$QAT_DC_EFFECTIVE_SHAPE_STATS"
	    [zfs_qat_dc_timing_stats]="$QAT_DC_TIMING_STATS"
	    [zfs_qat_dc_effective_timing_stats]="$QAT_DC_EFFECTIVE_TIMING_STATS"
